...
public static class SqliteData
{
    ...
    public static void CreateDatabase()
    {
        if (!File.Exists(Path.Combine(Configuration.DataDir, "GameDB.sqlite")))
        {
            ...
            var createTableSql =
                "CREATE TABLE Games (GameID INTEGER PRIMARY KEY, GameName TEXT, Developer TEXT, ReleaseDate DATE, Genre TEXT, Type TEXT, FilePath TEXT)";
        ...
        string? path) // method for adding sample data
    {
        using (var connection = new SQLiteConnection($"Data Source={Datapath}/GameDB.sqlite"))
        {
            ...
            // Insert game data
            var insertSql =
                "INSERT INTO Games (GameName, Developer, ReleaseDate, Genre, Type, FilePath) VALUES (@GameName, @Developer, @ReleaseDate, @Genre, @Type, @FilePath)";
